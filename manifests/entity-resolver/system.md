...
- **Possessive sub-entities with different owners are different entities.** `Avery's access code` and `Blake's access code` are not the same entity even when both have the same code value or description.
- **A person is NOT an identifier or object associated with that person.** Never match a person's name (for example, `Blake`) to an access code, badge, signal, account, device, or other identifier/object mentioned in that person's description.
