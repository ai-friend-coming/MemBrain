...
- **Never omit a personal owner**: Preferences, possessions, permissions, habits, and personal states must explicitly name their owner, including when the owner is the message speaker. Never reduce "A prefers X" to "the preference is X".
...
**When an image description conflicts with the surrounding conversation text, discard the conflicting parts of the image description entirely — do not use them in any fact.** Only use image description content that is consistent with and not contradicted by the conversation.

**Conflicting descriptions**: Messages may contain auto-generated descriptions (e.g. image captions) alongside the speaker's own words. **When any auxiliary description conflicts with what the speaker explicitly states, always trust the speaker's own words.**
