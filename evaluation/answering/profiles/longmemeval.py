"""LongMemEval dataset evaluation profile."""
...
_SSA_RE = re.compile(
    r"remind me|our previous|back to our|that you\b|trying to recall|you recommended",
    re.IGNORECASE,
)


class LongMemEvalProfile(BaseEvalProfile):
    ...
    def retrieve(
        ...
        if _SSA_RE.search(qa.question):
            return client.search_ssa(
                task_pk=task_pk,
    ...
    def generate_answer(
        ...
        agent, agent_settings = self._factory.get_agent(
            "qa-answerer", profile="longmemeval"
        )
